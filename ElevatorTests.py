...
class Elevator:
    ...
    def emergency_stop(self) -> bool:
        ...
        print(f"[Elevator {self.id}] 🚨 EMERGENCY STOP ACTIVATED")
        return True
    ...
    def update_weight(self, weight: float) -> bool:
        ...
        if self.overloaded:
            print(f"[Elevator {self.id}] ⚠️  OVERLOAD DETECTED: {weight}kg > {self.max_capacity}kg")
...
class ElevatorSystem:
    ...
    def call_elevator(self, floor: int, direction: Direction) -> int:
        ...
        if (direction == Direction.UP and floor == self.floors) or \
           (direction == Direction.DOWN and floor == 1):
            print(f"❌ Invalid direction for floor {floor}")
            return -1
        ...
        if best_elevator:
            ...
            print(f"[System] Floor {floor} {direction} → Elevator {best_elevator.id}")
    ...
    def print_status(self):
        ...
        for elevator_id, info in status.items():
            emergency = "🚨" if info['emergency'] else ""
            overload = "⚖️" if info['overloaded'] else ""
...
class TestDispatchingAlgorithm(unittest.TestCase):
    ...
    """Test that busy elevators are skipped for new calls"""
    # Make elevator 2 appear busy by adding a target floor
    self.system.elevators[2].target_floors.add(15)
    self.system.elevators[2].direction = Direction.UP
    
    # Call should go to nearest idle (elevator 1 at floor 1)
    elevator_id = self.system.call_elevator(10, Direction.UP)
    
    # Should NOT be elevator 2 (it has pending work)
    self.assertNotEqual(elevator_id, 2, 
                       "Busy elevator was incorrectly assigned")
    
    # Should be either 1 or 3
    self.assertIn(elevator_id, [1, 3], 
                  f"Expected elevator 1 or 3, got {elevator_id}")
...
def run_all_tests(verbosity=2):
    """Run all test suites"""
    print("🚀 Running Elevator System Test Suite")
    print("="*60)
...
def test_morning_rush():
    ...
    for i, dest in enumerate(destinations, 1):
        print(f"  Person {i}: Ground floor → Floor {dest}")
        elevator_id = system.call_elevator(1, Direction.UP)
        if elevator_id > 0:
            system.elevators[elevator_id].move_to_floor(dest)
            trips[elevator_id] += 1
...
def test_lunch_hour():
    ...
    for floor in [10, 15, 8]:
        print(f"  Floor {floor} → Cafeteria (Floor 2)")
        system.call_elevator(floor, Direction.DOWN)
        
    # Back up to offices
    for dest in [10, 12, 15]:
        print(f"  Cafeteria (Floor 2) → Floor {dest}")
...
def test_emergency():
    ...
    print("1. Normal call: Floor 5 → UP")
    system.call_elevator(5, Direction.UP)
    
    # Emergency!
    print("\n2. 🚨 EMERGENCY STOP ACTIVATED on Elevator 2!")
    system.elevators[2].emergency_stop()
    
    # Verify emergency state
    print("\n3. Verifying emergency state...")
    if not system.elevators[2].emergency_active:
        print("  ❌ Emergency not activated!")
        return False
        
    if not system.elevators[2].door_open:
        print("  ❌ Doors not open during emergency!")
        return False
        
    print("  ✅ Emergency properly activated")
    ...
    if elevator_id == 2:
        print("  ❌ System used emergency elevator!")
        return False
        
    print(f"  ✅ System correctly used Elevator {elevator_id}")
...
def test_overload():
    ...
    if overloaded:
        print("  ❌ False overload detection!")
        return False
    print("  ✅ Normal load accepted")
    
    # Overload
    print("\n2. Overload condition (900kg)")
    overloaded = system.elevators[1].update_weight(900)
    if not overloaded:
        print("  ❌ Overload not detected!")
        return False
    print("  ✅ Overload correctly detected")
    
    # Try to move overloaded elevator
    print("\n3. Attempting to move overloaded elevator...")
    floors = system.elevators[1].move_to_floor(10)
    if floors > 0:
        print("  ❌ Overloaded elevator moved!")
        return False
    print("  ✅ Overloaded elevator prevented from moving")
    ...
    can_close = system.elevators[1].close_doors()
    if can_close:
        print("  ❌ Doors closed despite overload!")
        return False
    print("  ✅ Doors prevented from closing when overloaded")
...
def test_performance():
    ...
    if avg_response > 100:  # 100ms threshold for simulated tests
        print("  ⚠️  Response time higher than expected")
        return False
        
    if successful < 15:
        print("  ⚠️  Throughput lower than expected")
        return False
            ...
            if command == "exit":
                ...
                if len(parts) == 3:
                    try:
                        ...
                        elevator_id = system.call_elevator(floor, direction)
                        if elevator_id > 0:
                            print(f"✅ Elevator {elevator_id} dispatched")
                        else:
                ...
                if len(parts) == 3:
                    try:
                        ...
                        if elevator_id in system.elevators:
                            ...
                            print(f"✅ Elevator {elevator_id} moving to floor {floor}")
                        else:
                            print(f"❌ Invalid elevator ID: {elevator_id}")
                    except:
                        print("❌ Usage: goto <elevator> <floor>")
                else:
                    print("❌ Usage: goto <elevator> <floor>")
                ...
                if len(parts) == 2:
                    try:
                        ...
                        if elevator_id in system.elevators:
                            ...
                            print(f"✅ Emergency stop activated on Elevator {elevator_id}")
                        else:
                            print(f"❌ Invalid elevator ID: {elevator_id}")
                    except:
                        print("❌ Usage: emergency <elevator>")
                else:
                    print("❌ Usage: emergency <elevator>")
                ...
                if len(parts) == 3:
                    try:
                        ...
                        if elevator_id in system.elevators:
                            ...
                            if overloaded:
                                print(f"⚠️  Elevator {elevator_id} OVERLOADED: {weight}kg")
                            else:
                                print(f"✅ Elevator {elevator_id} weight: {weight}kg")
                        else:
                            print(f"❌ Invalid elevator ID: {elevator_id}")
                    except:
                        print("❌ Usage: weight <elevator> <kg>")
                else:
                    print("❌ Usage: weight <elevator> <kg>")
